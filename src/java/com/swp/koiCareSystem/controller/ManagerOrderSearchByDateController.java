 ...
 * @author ASUS
 */
public class ManagerOrderSearchByDateController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            int count = os.countOrdersByDateRange(startDate, endDate);
            int endPage = count / 10;
            if (count % 10 != 0) {
                endPage++;
            }

            ArrayList<Order> listOrderSearchDateRange = os.searchOrdersByDateRange(startDate, endDate, index);
            ArrayList<String> listCustomerName = os.getListCustomerNames();
            ArrayList<OrderStatus> listStatus = os.getAllOrderStatuses();

            request.setAttribute("ListO", listOrderSearchDateRange);
